...
export function buildStepPlaceholder(step) {
  ...
  // 标题/选题类步�?  if (output.includes('标题') || output.includes('选题') || output.includes('主题') || title.includes('标题')) {
    return '先写一句垃圾话当标题，比如：\n"这个话题我觉�?.."\n\n备选标�?2：\n备选标�?3�?;
  }

  // 提纲/框架类步�?  if (output.includes('提纲') || output.includes('框架') || title.includes('大纲')) {
    return '不用想结构，先把脑子里闪过的三个词写下来：\n\n�?1：\n�?2：\n�?3：\n\n然后再串起来�?;
  }

  // 开�?Hook�?  if (output.includes('开�?) || output.includes('hook') || title.includes('开�?) || title.includes('引言')) {
    return '先写一句废话当开头，比如：\n"今天想聊�?.."\n\n写完再改，现在只负责把字打出来�?;
  }
  ...
  // 邮件/消息�?  if (output.includes('邮件') || output.includes('消息') || output.includes('联系') || title.includes('邮件') || title.includes('消息')) {
...
export function renderStepInstruction(step) {
  ...
  const output = escapeHtml(step.output || '一个可见产�?);
  return `
...
export function updateStepSubmitState() {
  ...
  btn.disabled = !ready;
  btn.textContent = ready ? '做完了，下一�? : '�?5 个字后提�?;
}
...
export function goToStep(idx) {
  ...
  document.getElementById('stepTitle').textContent = step.title || `�?${idx + 1} 步`;
  ...
  // 碎纸机模式：乱写期视觉提�?  const stepPage = document.getElementById('pageStep');
  if (stepPage) stepPage.classList.add('shredder-mode');

  // 添加碎纸机徽章到步骤标题�?  let shredderBadge = document.querySelector('.shredder-badge');
  if (!shredderBadge) {
    shredderBadge = document.createElement('span');
    shredderBadge.className = 'shredder-badge';
    shredderBadge.textContent = '🗑�?碎纸�?;
...
async function fetchProactiveSuggestion(idx) {
  ...
  try {
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({
        message: `基于用户前序产出，为当前步骤给出一句具体的开场建议（不超�?0字）。不要复读步骤说明。\n\n前序产出：\n${context}\n\n当前步骤�?{step.title}\n要求产出�?{step.output}`,
        phase: 'step',
        task: currentTask,
        steps,
        current_step: idx,
...
export async function finishStep() {
  ...
  try {
    ...
    // 归档到破冰日�?    const step = steps[currentStepIdx];
